...
<template>
  <div class="bg-white/[0.03] border border-white/[0.07] rounded-2xl overflow-hidden">
    ...
    <div class="p-6">
      ...
      <form @submit.prevent="lookup" class="flex gap-2 mb-5">
        <div class="flex-1 relative">
          <input
            ...
            class="w-full bg-black/40 border border-white/[0.08] rounded-lg px-4 py-2.5 text-sm text-slate-200 font-mono placeholder-slate-600 focus:outline-none focus:border-orange-500/40 focus:ring-1 focus:ring-orange-500/20 transition-all"
          />
        </div>
        <button
          type="submit"
          :disabled="loading || !query.trim()"
          class="px-5 py-2.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all duration-200 flex items-center gap-2"
      ...
      <!-- Error -->
      <div v-if="error" class="flex items-center gap-2 p-3 rounded-lg bg-red-500/5 border border-red-500/20 text-red-400 text-[12px] mb-4">
        <span>⚠</span> {{ error }}
      </div>
